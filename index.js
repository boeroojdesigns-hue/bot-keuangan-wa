...
if (!GAS_URL || !MONGO_URI) {
    console.error("ERROR: Pastikan Environment Variable GAS_URL dan MONGO_URI sudah diisi di Render.");
    process.exit(1);
}
...
mongoose.connect(MONGO_URI).then(() => {
    ...
    client.on('message', async msg => {
        ...
        if (text.toLowerCase().startsWith('masuk ') || text.toLowerCase().startsWith('keluar ')) {
            ...
            // chat.sendStateTyping(); // Opsional, kadang bikin error di beberapa versi
